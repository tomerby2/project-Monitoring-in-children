...
%     dirPath_wave        = 'C:\Users\User\Documents\project\DATA\P1_ECG\';
    Files_wave          = dir([dirPath_wave, '*.csv']);
    dirPath_RR          = 'C:\Users\User\Documents\project\clean data\RR_diffs_ECG_250hz\';
%     dirPath_RR          = 'C:\Users\User\Documents\project\DATA\P1_ECG\RR\';
    ...
    %% find average and variance of RR distances
        cut_info2(ii,:)  = [tm(1), tm(end), 0, 0, 60/mean(RR_diffs), 60/var(RR_diffs), 60/max(RR_diffs), 60/min(RR_diffs),0]; % beats per minute
...
%     end
    ...
    [~,index]               = sort(cut_info2(:,1),'ascend');
    
    temp_info               = cut_info2;
    cut_info2                = temp_info(index,:);
    % classification according to "steps" in mean breaths 
    cut_info2(1:204,9)   = 1;
    cut_info2(205:367,9) = 2;    
    cut_info2(368:496,9) = 1;
    cut_info2(497:557,9) = 3;
...
%     cut_info(48:end,9) = 1;  
    ...
    % normalization & units
    cut_info2(:,3)       = (cut_info2(:,1)-cut_info2(1,1))/3600; % normalize by first sample
    cut_info2(:,4)       = (cut_info2(:,2)-cut_info2(1,1))/3600; % and change units to hours
    ...
    [EigVec, EigVal] = diffusion_map(cut_info2(:,5:8), 1);
    
    %% plot average heart rate
    figure; hold on; title('average heart rate in cut from ECG 250Hz');
    xlabel('time [hour]'); 
    ylabel('beats per minute'); 
    plot(mean(cut_info2(:,3:4),2), cut_info2(:,5), 'LineWidth', 1.5); 
    set(gca, 'FontSize', 24); 
    grid on;
    hold off;
    
    %% plotting diffusion map of histograms wave colored according to time
    figure; hold on; scatter3(EigVec_h_wave(:,2), EigVec_h_wave(:,3), EigVec_h_wave(:,4), 100, mean(cut_info2(:,3:4),2) ,'Fill'); c=colorbar;
    ...
    figure; hold on; scatter3(EigVec_h_wave(1:557,2), EigVec_h_wave(1:557,3), EigVec_h_wave(1:557,4), 100, cut_info2(1:557,9) ,'Fill'); c=colorbar;
    title('diffusion map on histograms of the full wave ');
    xlabel('\psi_1');
    ylabel('\psi_2');
    zlabel('\psi_3');
...
%     hold off;
    ...
    figure; hold on; scatter3(EigVec_h_RR(:,2), EigVec_h_RR(:,3), EigVec_h_RR(:,4), 100, mean(cut_info2(:,3:4),2) ,'Fill'); c=colorbar;
    ...
    figure; hold on; scatter3(EigVec_h_RR(1:557,2), EigVec_h_RR(1:557,3), EigVec_h_RR(1:557,4), 100, cut_info2(1:557,9) ,'Fill'); c=colorbar;
    title('diffusion map on histograms of the RR distances');
    xlabel('\psi_1');
    ylabel('\psi_2');
    zlabel('\psi_3');
...
%     hold off;
    ...
    figure; hold on; scatter3(EigVec_f(:,2), EigVec_f(:,3), EigVec_f(:,4), 100 ,mean(cut_info2(:,3:4),2), 'Fill'); c=colorbar;
    ...
    figure; hold on; scatter3(EigVec_f(1:557,2), EigVec_f(1:557,3), EigVec_f(1:557,4), 100 ,cut_info2(1:557,9), 'Fill'); c=colorbar;
    title('diffusion map of abs fft');
    xlabel('\psi_1');
    ylabel('\psi_2');
    zlabel('\psi_3');
...
%     hold off;
    ...
    figure; hold on; scatter3(EigVec(:,2), EigVec(:,3), EigVec(:,4), 100 ,mean(cut_info2(:,3:4),2), 'Fill'); colorbar;
