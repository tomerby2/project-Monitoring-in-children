...
for ii = 1 : L
    ...
    %% find average and variance of RR distances
        cut_info_ppg(ii,:)  = [tm(1), tm(end), 0, 0, 60/mean(cyc_diffs), 60/var(cyc_diffs), 60/max(cyc_diffs), 60/min(cyc_diffs),0]; % breaths per minute
        
    %% creating matrix with histogram rows for wave
        edges                   = linspace(1024,3071,100);
...
% %     end
    ...
    [~,index]               = sort(cut_info_ppg(:,1),'ascend');
    
    temp_info               = cut_info_ppg;
    cut_info_ppg                = temp_info(index,:);
...
%     cut_info(497:557,9) = 3;
    ...
    cut_info_ppg(1:47,9)   = 0;
    cut_info_ppg(48:end,9) = 1;  
...
%     cut_info                = cut_info_complete(1:310,:);
    ...
    % normalization & units
    cut_info_ppg(:,3)       = (cut_info_ppg(:,1)-cut_info_ppg(1,1))/3600; % normalize by first sample
    cut_info_ppg(:,4)       = (cut_info_ppg(:,2)-cut_info_ppg(1,1))/3600; % and change units to hours
    ...
    [EigVec, EigVal]            = diffusion_map(cut_info_ppg(:,5:8), 4);
    ...
    figure; hold on; title('average heart rate in cut (5 minutes) from PLETH');
    xlabel('time [hour]'); 
    ylabel('beats per minute'); 
    plot(mean(cut_info_ppg(:,3:4),2), cut_info_ppg(:,5), 'LineWidth', 1); 
    set(gca, 'FontSize', 24); 
    grid on;
    hold off;
    
    %% plotting diffusion map of histograms wave colored according to time
    figure; hold on; scatter3(EigVec_h_wave(:,2), EigVec_h_wave(:,3), EigVec_h_wave(:,4), 100, mean(cut_info_ppg(:,3:4),2) ,'Fill'); c=colorbar;
    ...
    figure; hold on; scatter3(EigVec_h_wave(:,2), EigVec_h_wave(:,3), EigVec_h_wave(:,4), 100, cut_info_ppg(:,9) ,'Fill'); c=colorbar;
    title('diffusion map on histograms of the full wave ');
    xlabel('\psi_1');
    ylabel('\psi_2');
    zlabel('\psi_3');
    ...
    figure; hold on; scatter3(EigVec_h_cyc(:,2), EigVec_h_cyc(:,3), EigVec_h_cyc(:,4), 100, mean(cut_info_ppg(:,3:4),2) ,'Fill'); c=colorbar;
    ...
    figure; hold on; scatter3(EigVec_h_cyc(:,2), EigVec_h_cyc(:,3), EigVec_h_cyc(:,4), 100, cut_info_ppg(:,9) ,'Fill'); c=colorbar;
    title('diffusion map on histograms of the cycle distances');
    xlabel('\psi_1');
    ylabel('\psi_2');
    zlabel('\psi_3');
    ...
    figure; hold on; scatter3(EigVec_f(:,2), EigVec_f(:,3), EigVec_f(:,4), 100 ,mean(cut_info_ppg(:,3:4),2), 'Fill'); c=colorbar;
    ...
    figure; hold on; scatter3(EigVec_f(:,2), EigVec_f(:,3), EigVec_f(:,4), 100 ,cut_info_ppg(:,9), 'Fill'); c=colorbar;
    title('diffusion map of abs fft');
    xlabel('\psi_1');
    ylabel('\psi_2');
    zlabel('\psi_3');
    ...
    figure; hold on; scatter3(EigVec(:,2), EigVec(:,3), EigVec(:,4), 100 ,mean(cut_info_ppg(:,3:4),2), 'Fill'); colorbar;
